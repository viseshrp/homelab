...
| [FAQ guidance](https://crazymax.dev/diun/faq/) describes fixed tags, digest-only references, and repository discovery | Fixed/local-build gaps have exact rules; new majors use narrowly filtered repository rules; intended non-`latest` channels are explicit. |
