...
The lab currently maps 29 Compose projects to eight deployment hosts. Public web traffic enters through Nginx Proxy Manager or the dedicated Home Assistant Cloudflare Tunnel; storage-heavy media services live on the OptiPlex; web apps and automation share `rpiblog`; monitoring, notifications, archives, and document services share `rpimon`. DIUN runs independently on every Docker host, combining local inventory discovery with that host's tailored release rules and publishing notification-only results through ntfy.
...
- [`home-assistant-diun-agent/`](home-assistant-diun-agent) packages DIUN as a Home Assistant OS app.
...
home-assistant-diun-agent/    Home Assistant OS packaging for the DIUN agent
